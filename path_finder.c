...
void    recursive_path_move(t_path *path, int x, int y)
{
    ...
    if (y < path->height - 1 && path->map[y + 1][x] != '1' && path->map[y + 1][x] != 'X') {
        recursive_path_move(path, x, y + 1);
    }
    if (y > 0 && path->map[y - 1][x] != '1' && path->map[y - 1][x] != 'X') {
        recursive_path_move(path, x, y - 1);
    }
    if (x < path->width - 1 && path->map[y][x + 1] != '1' && path->map[y][x + 1] != 'X') {
        recursive_path_move(path, x + 1, y);
    }
}
...
void path_check(t_path *path, t_game *data)
{
    ...
    while (path->map[++i])
    {
        ...
        while (path->map[i][++j])
        {
            if ((data->map[i][j] == 'C' || data->map[i][j] == 'E') && path->map[i][j] == 'X')
                ce_count--;
        }
    }
    if (ce_count == 0)
